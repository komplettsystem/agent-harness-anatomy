...
Keep two things in mind on every memory conversation. The **Google entry is the lowest-confidence entry in this map** — Google has been renaming its agent products, recent documentation refers to an "Agent Platform SDK," and current naming, native evaluation offering and Memory Bank residency guarantees all need verifying before you cite any of it. And **erasure across a derivation boundary is unresolved.** Record-level deletion is documented for three of the four platforms (checked 2026-09-21). Claude Managed Agents has `memories.delete` plus `memory_versions.redact` for historical versions. AWS AgentCore's `DeleteMemoryRecord` works on the same extracted records its strategies produce. Foundry documents item-level delete across user-profile, chat-summary and procedural memory, though memory is still in preview. Google's own Memory Bank governance tutorial does not address erasure, and its primary revisions page would not load when I checked, so that entry stays open. What none of them documents is erasure that crosses a derivation boundary. Claude's dreams never modify their input store: they write a new, separate output store, so deleting a memory in the source store does not reach a dream output that already absorbed it. Whether deleting a source record on AWS also removes a summary derived from it is not stated in the docs I read. A skill synthesised from a person's data has the same shape, a separate artefact that outlives its source; that follows from how skills are stored, not from any vendor statement.
